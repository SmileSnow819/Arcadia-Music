      ...
      <v-card variant="text" v-intersect="onIntersect">
        ...
        <template #subtitle>
          <div class="playlist-desc-container">
            ...
            <div>
              <span class="playlist-creator">创建者：{{ creator.nickname }}</span>
        ...
        <template #actions>
          ...
          <v-btn
            v-if="!hasThisPlaylist"
            variant="elevated"
            icon
            size="large"
            @click="FCollectPlaylist"
          >
            <v-icon icon="mdi-music-note-plus" size="x-large"> </v-icon>
          ...
          <v-btn
            ...
            icon
            size="large"
            @click="downloadTrack([...currentPageTracks], baseDir)"
          >
          ...
          >
            ...
            <v-menu>
              ...
              <v-list>
                ...
                <v-list-item v-if="hasThisPlaylist" @click="FDeleteTrack(item.id)">
                  <v-list-item-title>
                ...
                <v-list-item v-if="hasThisPlaylist" @click="FMoveTrack(item.id)">
                  <v-list-item-title>
                    <v-icon :icon="items[3].icon"></v-icon>
                    {{ items[3].title }}
                  </v-list-item-title>
                </v-list-item>
              </v-list>
...
<script setup lang="ts">
  ...
  let totalTracks = [] as any
  const creator = shallowRef({}) as any
  const fab = ref(true)
  const page = ref(1)
  ...
  let checkIDS = []
  //如果是自己喜欢的音乐，不显示红心
  const showHeart = userStore.netEaseCloud.playlists[0].id !== playlistId
  const increment = 200
  const items = [
  ...
  let checkDone = ref(false)
  const hasThisPlaylist = userStore.netEaseCloud.playlists.some(
  ...
  async function FGetPageTrack(newPage: number): Promise<void> {
    ...
    //如果之前没加载过，就加载
    if (!totalTracks[start].id) {
      const ids = playlist.value.trackIds.slice(start, end)

      await FCheckHeart(checkIDS.slice(start, end))
      const res = await window.api.netEaseCloud.song_detail({ ids })
      currentPageTracks.splice(0, currentPageTracks.length, ...res.songs)
      totalTracks.splice(start, increment, ...res.songs)
    } else {
      //如果之前加载过，就直接拿来用
  ...
  window.api.netEaseCloud.playlist_detail(options).then(async (res: any) => {
    playlist.value = res.playlist
    checkIDS = playlist.value.trackIds.map((item: any) => item.id)
    pageLength = Math.ceil(playlist.value.trackCount / increment)
    totalTracks = new Array(playlist.value.trackCount).fill({})
    // FGetPageTrack(1)
    const start = 0
    const end = increment
    await FCheckHeart(checkIDS.slice(start, end))
    currentPageTracks.splice(
      0,
      currentPageTracks.length,
      ...res.playlist.tracks.slice(0, increment)
    )
    totalTracks.splice(start, increment, ...res.playlist.tracks)
    nextTick(() => {
      pageChange.value = true
      realPage.value = 1
    })
  ...
  function FPlayAll(event: any) {
    if (player.listID !== `playlist-${playlistId}-${page.value}`)
      stateStore.tip = '播放歌单:' + playlist.value.name
    player.loadList(currentPageTracks, `playlist-${playlistId}-${page.value}`)
    if (event === 0) {
  ...
  //删除歌曲
  function FDeleteTrack(id: number) {
    // 重新获取歌单，并在3秒后更新
    window.api.netEaseCloud.playlist_track('del', playlistId, [id]).then(() => {
      setTimeout(() => {
        //重新获得歌单，并更新当前歌单
        userStore.getNetEaseCloudPlaylist().then(() => {
          playlist.value = userStore.netEaseCloud.playlists.find((pl) => pl.id === playlistId)
        })
      }, 1000)
    })
    // 从歌单里删除这首歌
    currentPageTracks.splice(
      currentPageTracks.findIndex((track) => track.id === id),
      1
    )
    // 从总歌单里删除这首歌
    totalTracks.splice(
      totalTracks.findIndex((track) => track.id === id),
      1
    )
  }
  ...
  function FChangeIndex() {
    ...
    if (offsetIndex + currentIndex < 0) offsetIndex = -currentIndex
    //拿到这首歌
  ...
  function FOrderUpdate() {
    if (hasThisPlaylist && currentIndex !== -1) {
      window.api.netEaseCloud.song_order_update(
        playlistId,
        currentPageTracks.map((item) => item.id)
      )
    }
  }
